...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order_formatted = [int(a) for a in order.to_numpy().tolist()]
        ctx.save_for_backward(order_formatted)
        a_new = a._new(a._tensor.permute(*order_formatted))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        back_order = np.zeros_like(order)
        for (i, val) in enumerate(order):
            back_order[order[i]] = i

        return grad_output._new(grad_output._tensor.permute(*back_order)), 0.0
